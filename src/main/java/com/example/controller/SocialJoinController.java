...
public class SocialJoinController {
    ...
    public ResponseEntity<?> socialJoin(@ModelAttribute JoinDTO joinDTO, HttpServletResponse response) {
        ...
        // 2. 가입된 사용자 조회
        MemberVO user = userRepository.findBySocialIdAndSocialType(
            joinDTO.getSocialId(), joinDTO.getSocialType()
        ).orElseThrow(() -> new RuntimeException("회원가입 후 사용자 조회 실패"));

        // 3. JWT 생성
        String token = jwtUtil.createJwt(
            user.getSocialId(), // username 대체용
            user.getNickname(),
            user.getRoles(),    // "ROLE_MENTOR" 같은 값
            1000L * 60 * 60 * 24 // 24시간
        );
